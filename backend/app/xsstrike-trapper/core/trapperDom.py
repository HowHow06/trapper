    ...
    for script in scripts:
        ...
        try:
            for newLine in script:
                ...
                for grp in pattern:
                    if grp:
                        source = newLine[grp.start():grp.end()
                                         ].replace(' ', '')
                        if source:
                            if len(parts) > 1:
                                for part in parts:
                ...
                for controlledVariable in allControlledVariables:
                    matches = list(filter(None, re.findall(
                        r'\b%s\b' % controlledVariable, line)))
                    if matches:
                        sourceFound = True
                        line = re.sub(r'\b%s\b' % controlledVariable,
                                      yellow + controlledVariable + end, line)
                ...
                for grp in pattern:
                    if grp:
                        ...
                        if sink:
                            ...
                            sinkFound = True
                if line != newLine:
                    highlightedInScript.append('%s' % (line.lstrip(' ')))
                num += 1
        except MemoryError:
            pass
        if len(highlightedInScript) > 0:
            lineBreak = "\n"
            highlighted.append(lineBreak.join(highlightedInScript))
