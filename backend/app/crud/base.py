...
class CRUDBaseSync(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100, where=None, sort_by: Optional[str] = None,
                  desc_order: bool = False, join=None) -> List[ModelType]:
        conditions = [self.model.deleted_at.is_(None)]
        if where is not None:
            conditions.append(where)
    ...
    def update_by_where(self, db: Session, values: Dict[str, any], where=None) -> int:
        if not values:
            return 0
