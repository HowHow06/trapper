...
async function set_up_api_server(app) {
  ...
  app.use(
    '/admin/',
    express.static('./front-end/dist/', {
      setHeaders: function (res, path, stat) {
        res.set(
          'Content-Security-Policy',
          "default-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self'; font-src 'self'; connect-src 'self'; prefetch-src 'self'; manifest-src 'self'"
        );
      },
    })
  );
  ...
  app.delete(
    constants.API_BASE_PATH + 'payloadfires',
    validate({ body: DeletePayloadFiresSchema }),
    async (req, res) => {
      ...
      await Promise.all(
        screenshots_to_delete.map((screenshot_path) => {
  ...
  app.get(
    constants.API_BASE_PATH + 'payloadfires',
    validate({ query: ListPayloadFiresSchema }),
    async (req, res) => {
  ...
  app.get(
    constants.API_BASE_PATH + 'collected_pages',
    validate({ query: ListCollectedPagesSchema }),
    async (req, res) => {
      const page = parseInt(req.query.page) - 1;
      const limit = parseInt(req.query.limit);
      const offset = page * limit;
  ...
  app.delete(
    constants.API_BASE_PATH + 'collected_pages',
    validate({ body: DeleteCollectedPagesSchema }),
    async (req, res) => {
      const ids_to_delete = req.body.ids;
  ...
  app.post(
    constants.API_BASE_PATH + 'record_injection',
    validate({ body: RecordCorrelatedRequestSchema }),
    async (req, res) => {
      ...
      if (
        !safeCompare(
          correlation_key_record.value,
          req.body.owner_correlation_key
        )
      ) {
        res
          .status(200)
          .json({
            success: false,
            error:
              'Invalid authentication provided. Please provide a proper correlation API key.',
            code: 'INVALID_CREDENTIALS',
          })
          .end();
        return;
  ...
  app.get(constants.API_BASE_PATH + 'settings', async (req, res) => {
    ...
    let result = {};
    let database_promises = settings_to_retrieve.map(
      async (settings_value_metadata) => {
        ...
        const formatter_function = settings_value_metadata.formatter
          ? settings_value_metadata.formatter
          : (value) => value;
        result[settings_value_metadata.return_key] = db_record
          ? formatter_function(db_record.value)
          : settings_value_metadata.default;
      }
    );
