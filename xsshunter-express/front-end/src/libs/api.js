...
async function is_authenticated() {
  return api_request('GET', `/api/v1/auth-check`, false);
}

async function authenticate(password) {
  return api_request('POST', `/api/v1/login`, {
    password: password,
  });
}
...
async function delete_payload_fires(payload_id_array) {
  return api_request('DELETE', `/api/v1/payloadfires`, {
...
async function delete_collect_pages(collected_pages_id_array) {
  return api_request('DELETE', `/api/v1/collected_pages`, {
    ids: collected_pages_id_array,
  });
}

async function get_settings() {
  return api_request('GET', `/api/v1/settings`, false);
}

async function update_password(new_password) {
  return api_request('PUT', `/api/v1/settings`, {
